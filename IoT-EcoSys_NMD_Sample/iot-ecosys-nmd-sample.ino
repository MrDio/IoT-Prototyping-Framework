...
#define LED_PIN D7
#define AUDIO_BUFFER_MAX 600 //255
...
void setup() {
  ...
  // Wait for USB Serial 
   ...
   pinMode(BUTTON_PIN, INPUT);
    ...
    if (!myFile.open("Messungen.csv", O_RDWR | O_CREAT | O_AT_END)) {
        if(debug){
            Particle.publish("[Debug] Opening Messung.csv for write failed", 0);        
        }
        sd.errorHalt("opening Messung.csv for write failed");
    }else{
        myFile.println(" counter ; timestamp ; amplitude ; accx ; accy ; accz ; magx ; magy ; magz ; gyrx ; gyry ; gyrz ; temp ; act state");
        if(debug){
